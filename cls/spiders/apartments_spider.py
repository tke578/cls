...
class JobsSpider(scrapy.Spider):
    ...
    def parse_page(self, response):
        ...
        price = response.xpath('//span[@class="postingtitletext"]/span[@class="price"]/text()').extract_first()
        employment_type = response.xpath('//p[@class="attrgroup"]/span[2]/b/text()').extract_first()
        # posting_body = response.xpath('//section[@id="postingbody"]/b/text()').extract_first()
        posting_id = response.xpath('//div[@class="postinginfos"]/p[@class="postinginfo"]/text()').extract_first()[9:]
        posting_time = response.xpath('//div[@class="postinginfos"]/p[2]/time/text()').extract_first()
        updated_posting_time = response.xpath('//div[@class="postinginfos"]/p[3]/time/text()').extract_first() or 'n/a'
        uuid = hashlib.md5((posting_id + posting_time + updated_posting_time + sanitize_desc).encode('utf-8')).hexdigest()

        yield{'post_id': posting_id, 'post_time': posting_time, 'updated_post_time': updated_posting_time, 'url': url, 
        		'title': title, 'address': address, 'price': price, 'description': sanitize_desc, 'post_status': 'new',
                'region': 'San Francisco', 'state': 'CA', 'uuid': uuid}
